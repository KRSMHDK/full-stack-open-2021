...
const Blog = ({ blog, user, handleLikes, handleRemove }) => {
  ...
  return (
    <div className='blog'>
      <div className='compactView' style={(blogStyle, hideWhenExpanded)}>
        {blog.title}
        {blog.author}
        <button onClick={() => setExpanded(!expanded)}>view</button>
      </div>
      <div className='detailedView' style={(blogStyle, showWhenExpanded)}>
        ...
        <br />
        <button type='button' value={id} onClick={handleRemove}>
          remove
        </button>
      </div>
    </div>
